...
int main() {
    ...
    int all_trailhead_ratings_sum = 0;
    ...
    if(my_file.is_open()) {
        ...
        for(const & t : trailheads) {
            all_trailhead_ratings_sum += exploreTrailhead(topo_map, unhash_coord(topo_map.size(), t));
        }

    }

    std::cout << all_trailhead_ratings_sum << std::endl;
